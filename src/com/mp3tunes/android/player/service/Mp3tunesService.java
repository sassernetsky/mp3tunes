...
public class Mp3tunesService extends Service
{
    ...
    private static final int NOTIFY_ID = 10911251; // mp3
    // in
    // ascii
    ...
    public void onCreate()
    {
        ...
        mLocker = (Locker) MP3tunesApplication.getInstance().map
                .get("mp3tunes_locker");
    ...
    PhoneStateListener mPhoneStateListener = new PhoneStateListener() {
        ...
        public void onCallStateChanged(int state, String incomingNumber)
        {
            ...
            if (state == TelephonyManager.CALL_STATE_IDLE) // fade music in to
                                                           // 100%
            {
                mFadeVolumeTask = new FadeVolumeTask(FadeVolumeTask.FADE_IN,
                        5000) {
                    @Override
                    public void onPreExecute()
                    {
                        if (mServiceState == STATE.SYSTEMPAUSE)
                ...
                mFadeVolumeTask = new FadeVolumeTask(FadeVolumeTask.FADE_OUT,
                        duration) {
                    ...
                    public void onPostExecute()
                    {
                        Mp3tunesService.this.pause(STATE.SYSTEMPAUSE);
                        mFadeVolumeTask = null;
                    }
                };
            }
    ...
    private void playingNotify()
    {
        ...
        if (mCurrentTrack == null)
            return;
        Notification notification = new Notification(R.drawable.logo_statusbar,
                "Playing: " + mCurrentTrack.getTitle() + " by "
                        + mCurrentTrack.getArtistName(), System
                        .currentTimeMillis());
        ...
        public void onBufferingUpdate(MediaPlayer mp, int percent)
        {
            ...
            if (mNextMp == null && percent == 100) {
                ...
                if (mCp.getQueueSize() > mCurrentPosition) {
                    ...
                    mNextTrack = mCp.getTrackQueue(mNextPosition);
    ...
    {
        try {
            if (/* mServiceState == STATE.STOPPED || */mServiceState == STATE.PREPARING
                    || track == null)
                return;
    ...
    private int chooseBitrate()
    {
        int bitrate = Integer.valueOf(PreferenceManager
                .getDefaultSharedPreferences(this).getString("bitrate", "-1"));
    ...
    private void pause(int state)
    {
        ...
        if (mServiceState == STATE.PAUSED && state == STATE.SYSTEMPAUSE)
            return;
        ...
        // if ( mServiceState != STATE.PAUSED)
        {
            if (mActive == false)
                mDeferredStopHandler.deferredStopSelf();
            Notification notification = new Notification(R.drawable.right_play,
                    "MP3tunes Paused", System.currentTimeMillis());
            PendingIntent contentIntent = PendingIntent.getActivity(this, 0,
                    new Intent(this, Player.class), 0);
            String info;
            String name;
    ...
    private final ITunesService.Stub mBinder = new ITunesService.Stub() {
        ...
        public void start() throws RemoteException
        {
            ...
            playTrack(mCp.getTrackQueue(1), 1, mMp);
            notifyChange(META_CHANGED);
        }
        ...
        public void startAt(int pos) throws RemoteException
        {
            if (mCp.getQueueSize() < pos)
                throw new RemoteException();
            playTrack(mCp.getTrackQueue(pos), pos, mMp);
            notifyChange(META_CHANGED);
        ...
        public String[] getMetadata() throws RemoteException
        {
            ...
            if (mCurrentTrack == null)
                ...
                data = new String[] { mCurrentTrack.getTitle(),
                        Integer.toString(mCurrentTrack.getId()),
                        mCurrentTrack.getArtistName(),
                        Integer.toString(mCurrentTrack.getArtistId()),
                        mCurrentTrack.getAlbumTitle(),
                        Integer.toString(mCurrentTrack.getAlbumId()) };
        ...
        {
            mCp.moveQueueItem(index1++, index2++); // ++
            // because
            // sqlite
            // is
            // 1
        ...
        {
            int numremoved = mCp.removeQueueItem(first, last);
    ...
    private class RefreshSessionTask extends AsyncTask<Void, Void, Boolean>
    {
        ...
        public Boolean doInBackground(Void... input)
        {
            boolean success = false;
            try {
                SharedPreferences settings = getSharedPreferences(Login.PREFS,
                        0);
